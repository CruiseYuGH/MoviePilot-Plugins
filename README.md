...
3. 选择下注模式：
   ...
   - **反打**：大/小同侧连续达到配置次数后下注反侧；未中继续追同一侧，达到最多追手后停止
4. 下注金额：`100` ~ `100000`
5. 反打模式可配置：反打触发连续次数（默认 `5`）、反打最多追手（默认 `8`）
6. 可选限制：每日最大下注次数、每日观影券次数（评论出现「观影随机续期奖励」计 1 次，按自然天）
7. 建议执行周期：`*/3 * * * *`
8. 插件详情页可查看下注记录，以及日/周/月魔力盈亏汇总
