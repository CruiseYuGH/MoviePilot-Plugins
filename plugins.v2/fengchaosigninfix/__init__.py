...
class FengchaoSigninFix(_PluginBase):
    ...
    def init_plugin(self, config: dict = None):
        ...
        # 签到 cron 仅通过 get_service() 注册到 MoviePilot 主调度器，避免与内部调度器重复触发
        legacy_signin_job_id = "fengchao_signin_cron"
        if self._scheduler.get_job(legacy_signin_job_id):
            self._scheduler.remove_job(legacy_signin_job_id)
            logger.info("已移除内部调度器上的旧签到周期任务（改由 MoviePilot 主调度器统一管理）")
        if self._enabled != self._active_enabled and self._enabled and self._cron:
            logger.info(f"签到周期任务由 MoviePilot 主调度器管理，周期：{self._cron}")
